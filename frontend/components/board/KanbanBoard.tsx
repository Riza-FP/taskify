...
export function KanbanBoard({ boardId }: KanbanBoardProps) {
  ...
  function onDragEnd(event: DragEndEvent) {
    ...
    if (over.data.current?.type === "Column") {
      ...
      if (newListId === activeTask.list_id) return;

      const targetTasks = tasks
        .filter((t) => t.list_id === newListId && t.id !== activeTask.id)
    ...
    if (over.data.current?.type === "Task") {
      ...
      const targetTasks = tasks
        .filter((t) => t.list_id === targetListId && t.id !== activeTask.id)
        .sort((a, b) => compareLexorank(a.rank, b.rank));
            ...
            <SortableContext items={columnIds}>
              {columns.map((col) => (
                <BoardColumn
                  ...
                  column={col}
                  tasks={tasks
                    .filter((task) => task.list_id === Number(col.id))
                    .sort((a, b) => compareLexorank(a.rank, b.rank))}
            ...
            <DragOverlay>
              {activeColumn && (
                <BoardColumn
                  column={activeColumn}
                  tasks={tasks.filter(
                    (task) => task.list_id === Number(activeColumn.id),
                  )}
                  createTask={() => {}}
                  onTaskClick={() => {}}
                />
